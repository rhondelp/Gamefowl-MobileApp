...
| BUG-4 (**backend**, found during M14 admin-flow testing) | Removing or re-weighting a knowledge-base rule failed: the app called `/admin/rules/null`. Root cause in the API repo: `Disease::symptoms()` / `Symptom::diseases()` declared `->withPivot('weight')` without `'id'`, so `AdminDiseaseResource.rule_id` serialized as `null` and rule-targeted endpoints were unaddressable by any client. Unit tests missed it because they addressed rules via direct model lookups, never through the resource mapping. | Fix belongs to the **backend repo** (per project rules, untouched from this repo's sessions): change both declarations to `->withPivot('id', 'weight')`. Mobile side required no change — it correctly consumed the documented contract. |
